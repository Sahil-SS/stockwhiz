...
import { getAllUsersForNews } from "../actions/user.action";
  ...
  async ({ event, step }) => {
    ...
    await step.run("send-welcome-email", async () => {
      ...
      const introText =
        ...
        "Welcome to StockWhiz! We're excited to have you on board.";
...
export const sendDailyNewsletter = inngest.createFunction(
  ...
  async ({ event, step }) => {
    //Get all the users for newsletter from DB
    const users = await step.run("get-all-users", getAllUsersForNews);
    if (!users || users.length === 0) {
      return { success: false, message: "No users found for newsletter." };
    }
    //Fetch news for each user based on their preferences
    ...
    //Generate personalized news summary using AI
